...
def preprocess_data(df):
    # (Your original preprocessing logic remains exactly the same)
...
st.title("An improved photovoltaic power production forecasting using a hybrid model of Bi-LSTM and Informer Attention Mechanism")
...
if start_datetime < data_min_date or end_datetime > data_max_date:
    ...
    if st.button('Process Prediction'):
        # Work on a copy of the dataframe to avoid caching issues
        predict_df = df.copy()
        predict_df['date'] = pd.to_datetime(predict_df['date'])
        predict_df = predict_df[(predict_df['date'] >= start_datetime) & (predict_df['date'] <= end_datetime)]
